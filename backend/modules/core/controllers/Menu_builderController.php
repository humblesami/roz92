...
class Menu_builderController extends BaseController {
	...
	function build_menu($menu_data_row,$parent_child){
		...
		$menu_ul = '<ol class="dd-list">';
			...
			foreach($menu_data_row as $key => $value){
					...
					$menu_ul .= '<div class="panel-tools"><a class="btn btn-xs collapses" href="javascript:void(0)" onclick="expand_collaps(' . $feature_id . ',' . $feature_id . ',\'' . $menu_detail['name']. '\',\'' . $menu_name . '\')" ></a></div></li>
					';
	...
	*/		
	function actionCreate_menu_save(){
		$data = $_POST;
		$menu_mod = new TblCoreMenus();
		$menu_id = $menu_mod->create_menu_save($data,'B');
