! function(a) {
    var b = 0,
        ...
        d = {
            ...
            init: function(e, f) {
                ...
                if (!e) {
                    ...
                    f = a.extend(!0, d.defaults, f), c[b] = {
                        ...
                        area: [],
                        options: f
                    }, e = b, h.data("imageMapper", b++), h.addClass("image-mapper"), h.html('<img class="image-mapper-img"  /><svg class="image-mapper-svg"></svg>');
            ...
            custompoint: function(b,nx,ny) {
                
                var e = this,
                    f = a(this);
                   
               var nf = {naturalX: nx, naturalY: ny};
               d.addPoint.apply(e, [b, nf]), "function" == typeof c[b].options.event.update && c[b].options.event.update.apply(e)
...
/*                return {
            ...
            addArea: function(b, nd) {
             
                1 == arguments.length && (
                    nd = b, b = a(this).data("imageMapper")), 
            ...
            renderSVG: function(b) {
                ...
                //f.css("width", "100%"),  a.each(c[b].area, function(g, h) {
                    var i, j = [];
                    var t = [];
                    ...
                    if(j.length >= 4 ){
                        t = a(document.createElementNS("http://www.w3.org/2000/svg", "text"));
                        t.attr("data-area-index", g);
                        //t.css(c[b].options.style);
                       // t.css('writing-mode','tb')
                        t.css('fill','#000');
                        t.css('stroke','#ff0000');
                       

                        //t.attr("points", j.join(","))
                        t.attr("x", j[0] + 10).attr("y", j[1] + 10 );
                        t.attr("class", "image-mapper-shape")
                       // t.attr('fill','#fff');
                        // t.attr('stroke','#000000');
                       
                        // t.attr("r", Math.sqrt(Math.pow(j[2] - j[0], 2) + Math.pow(j[3] - j[1], 2)));
                        t.html('I D: '  + (g + 1));
                        f.prepend(t)
                    }
            ...
            renderPoints: function(b) {
                ...
                a.each(h.coords, function(h, i) {
                    ...
                    var j = a(document.createElementNS("http://www.w3.org/2000/svg", "circle")),
                       // t = a(document.createElementNS("http://www.w3.org/2000/svg", "circle")),
                        k = d.getClientPosition.apply(e, [b, i]);
                       // t.attr("cx", k.clientX).attr("cy", k.clientY), t.html("sdfsfasfdsr"),t.attr("r", 100), t.attr("class", "imsage-mapper-point"), t.attr("data-area-index", g), t.attr("data-coord-index", h), t.css(c[b].options.pointStyle), f.append(t),
...
function(a) {
    a(document).on("init", function() {
        ...
        a(document).on("click", "button.add-row", function(b) {
            ...
            $("tbody", c).html(""), a.each(b.area, function(nb) {
                ...
                d = d.replace(/x_\d+/gi, "x_" + nb)
               // d = d.replace('n_0','n_' + nb)
                d = d.replace('<tr id="x_"' + nb + '>','<tr id="x_' + nb + '">')
                d = d.replace(/im\[[\d]+\]/gi, "im[" + nb + "]"), a("tbody", c).append(d)

                $('#x_' + nb + ' .display_title').val(nb + 1);
      ...
      var txt='dd';
       // var coor= $(this).attr('coords');
        ...
        $span.appendTo('#image-map');
            ...
            0 == c.options.src.length ? a(".toggle-content").show() : a(".toggle-content").show()
